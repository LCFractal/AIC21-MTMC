...
class JDETracker(object):
    def __init__(self, conf_thre, frame_rate=30):
        ...
        self.frame_id = 0
        self.buffer_size = int(frame_rate / 30.0 * 30) # last 30 is tracker buffer in fairmot
    ...
    def update(self, dets, id_feature, frame_id):
        ...
        logger.debug('tracks info for cost matrix: ')
        for track in strack_pool:
            logger.debug('{}'.format((track.track_id, track.tlbr[0], track.tlbr[1], track.tlbr[2], track.tlbr[3])))
        logger.debug('embedding cost matrix: \n{}'.format(dists))
        ...
        ''' Step 3: Second association, with pixel dist(original code here is IOU, for person)'''
        detections = [detections[i] for i in u_detection]
        r_tracked_stracks = [strack_pool[i] for i in u_track if strack_pool[i].state == TrackState.Tracked]
        #dists = matching.iou_distance(r_tracked_stracks, detections)
        #matches, u_track, u_detection = matching.linear_assignment(dists, thresh=0.4)
        dists = matching.pixel_distance(r_tracked_stracks, detections)
        logger.debug('remained tracks info for cost matrix: {}'.format([(track.track_id, track.tlbr) for track in r_tracked_stracks]))
        logger.debug('pixel cost matrix: \n{}'.format(dists))
        ...
        for inew in u_detection:
            ...
            if track.score < self.conf_thres:
                continue
